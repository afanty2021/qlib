...
├─ 探测失败（网络/5xx/超时）
│   ├─ 任一镜像 200/206 → 视为已发布，走镜像下载（镜像链救援）
│   ├─ 镜像 404（透传型）→ 按未发布等待（exit 0，不告警）
│   └─ 直连与镜像均不可用 → 报错退出（exit 1，调度器可感知）
└─ 已上线 → 选镜像下载（镜像按序探测，全部失败回退直连；
            下载后 gzip 校验 + 直连元数据大小交叉校验，异常自动重试；
            每次重试重新选镜像）
    ↓
备份现有数据
    ↓
...
- **下载选路**：每次下载尝试按序探测镜像（首字节 range 请求，`-L` 跟随跳转，
  200/206 为可用），首个可用者胜出；全部不可用回退直连。禁用镜像加速请把数组
  置空（`MIRROR_PREFIXES=()`），勿注释/删除整块声明。
- **救援三态**：直连探测失败（HTTP 000/5xx，无法区分"未发布"）时用镜像面重新
  判定——任一镜像 200/206 → 视为已发布继续下载；透传型镜像 404 → 与直连 404
  同语义，按"未发布"静默等待（不告警）；直连与镜像全灭 → 才告警退出。
- **内容一致性是信任假设，不是保证**：镜像可能服务陈旧内容或损坏字节。两道闸门
  兜底——下载后先 `gzip -t` 完整性校验，再用直连响应头的包大小基准交叉校验
  （大小不符按损坏处理，删除重试）；数据日期早于目标日期时日志升 WARN 提示。
  要最强完整性保证，用自建 Worker 或直连。
- **镜像可用性变化频繁**（2026-09-22 实测 ghfast.top 可用、ghp.ci/gh-proxy.com/
  ghproxy.net/gh.llkk.cc 均不可用），列表按需增删即可，探测不通过自动跳过。
- **aria2c 侧**：`-x 16 -s 16 -k 1M` 并发分段（-x 16 为 aria2 硬上限）；
  `--lowest-speed-limit=10K` 掐死停滞连接（默认永不掐断会无限期挂死）——注意
  隐含下限：16 连接聚合吞吐低于 ~160KB/s 的链路会被判定停滞掐断，极慢直连日
  下载可能失败重试，属预期行为；`--timeout=30 --connect-timeout=10
  --retry-wait=2` 快速重建坏连接。
...
2. **重要**：`*.workers.dev` 在中国大陆被 DNS 污染/SNI 阻断（实测解析到假 IP），
   直连通常不可用。部署后必须在 Worker 的 Settings → Domains & Routes 绑定
   自有自定义域（Custom Domains）。
3. 把自定义域地址加到 `MIRROR_PREFIXES` 首位。
