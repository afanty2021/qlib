...
graph TD
    ...
    B --> B1["base.py - 基础抽象类"];
    B --> B2["data.py - 数据提供者实现"];
    B --> B3["client.py - 客户端访问"];
    B --> B4["pit.py - 点对点数据"];
    B --> B5["filter.py - 数据过滤"];
    B --> B6["inst_processor.py - 实例处理器"];

    C --> C1["cache.py - 缓存抽象"];
    C --> C2["ExpressionCache - 表达式缓存"];
    C --> C3["DatasetCache - 数据集缓存"];
    C --> C4["DiskCache - 磁盘缓存"];
    C --> C5["MemoryCache - 内存缓存"];

    D --> D1["ops.py - 数据操作符"];
    D --> D2["ElementOps - 元素操作"];
    D --> D3["SeriesOps - 序列操作"];
    D --> D4["ExpressionOps - 表达式操作"];

    click B1 "./base.py" "查看基础抽象类"
    click B2 "./data.py" "查看数据提供者实现"
    click C1 "./cache.py" "查看缓存抽象"
    click D1 "./ops.py" "查看数据操作符"
```
